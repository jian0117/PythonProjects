...
if __name__ == '__main__':
    ...
    schedule_strategy = 1
    supply_sku_infos = {"3000006675": {"supply_sku_price": 600.98, "supply_sku_delivery_area": 5},
                        "3000020017": {"supply_sku_price": 200.98, "supply_sku_delivery_area": 3}}
    # 下单收货地址
    ...
    if schedule_strategy == 1:
        ...
        if len(unique_prices) == 1:  # All prices are the same
            aspect_delivery_sku = -1
            print('供应sku的供货价都一样，随机取sku，不用校验；supply_sku_infos:{}'.format(supply_sku_infos))
        else:            min_price = min(supply_sku_infos.values(), key=lambda x: x["supply_sku_price"])["supply_sku_price"]
